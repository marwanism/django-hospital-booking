...
def createadminpage(request):
	...
	if request.method == 'POST':
		...
		try:
			if password == repeatpassword:
				...
				adm_group, created = Group.objects.get_or_create(name='Admins')
	...
	d = {'error' : error}
	#print(error)
	return render(request,'createadmin.html',d)
	...
	if request.method == 'POST':
		...
		try:
			if password == repeatpassword:
				...
				user = User.objects.create_user(first_name=name,email=email,password=password,username=email)
				rec_group, created = Group.objects.get_or_create(name='Receptionist')
...
def profile(request):
	...
	g = request.user.groups.all()[0].name
	if g == 'Patient':
		patient_details = Patient.objects.all().filter(email=request.user)[0]
	...
	if g == 'Patient':
		if request.method == 'POST':
			...
			try:
				...
				error = "no"
				print('Appointment created successfulyy')
